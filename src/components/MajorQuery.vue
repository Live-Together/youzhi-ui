			...
			<el-row :gutter="20">
				<el-col :span="8">
					<div class="grid-content bg-purple">
						<el-input v-model="target.majorName" placeholder="输入专业名称" @keyup.enter.native="handleSearch">
							<template #suffix>
				...
				<el-col :span="4">
					<div class="grid-content bg-purple">
						<el-select v-model="target.year" filterable placeholder="年份" clearable>
							<el-option v-for="item in filter.year" :key="item" :value="item"></el-option>
						</el-select>
					</div>
				</el-col>
				<el-col :span="4">
					...
					<div class="grid-content bg-purple">
						<el-select v-model="target.schoolVariety" placeholder="学校类别" clearable>
							<el-option v-for="item in filter.schoolVariety" :key="item" :value="item"
								:label="switchSchoolVariety(item)"></el-option>
						</el-select>
					</div>
				</el-col>
			</el-row>
		...
		methods: {
			handleSearch() {
				...
				if (this.target.majorName !== null && this.target.majorName !== '') {
					axios.get('/api/getSchoolInfoByMajorName', {
						params: {
