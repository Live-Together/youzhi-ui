<template>
	<div>
		<div id="main">
			<el-row :gutter="20">
				<el-col :span="7">
					<el-input v-model="schoolInfo.schoolName" class="inline-input" prefix-icon="el-icon-search"
						placeholder="请输入学校名称" @blur="getAllMajor"></el-input>
				</el-col>
				<el-col :span="7">
					<el-select v-model="schoolInfo.majorName" filterable placeholder="专业名称" clearable>
				...
				<el-table-column label="操作" align="center">
					<template #default="scope">
						<el-button type="danger" icon="el-icon-delete"
							@click="handleDelete(scope.$index + 1)"></el-button>
					</template>
				</el-table-column>
			</el-table>
		</div>
	...
	export default {
		...
		methods: {
			getAllMajor() {
				...
				axios("/api/getMajorNameBySchoolName", {
					params: {
			...
			handleSubmit() {
				if (this.schoolInfo.majorName === '' || this.schoolInfo.schoolName === '') {
				...
				let isReapet = this.tableData.every(item => {
					return item.majorName !== this.schoolInfo.majorName || item.schoolName !== this.schoolInfo
						.schoolName
				})
				if (!isReapet) {
				...
				axios.post("/api/" + this.studentId + "/schoolSubmit", this.schoolInfo).then(res => {
			...
			handleDelete(row) {
				...
				axios.post("/api/deleteSchoolSubmit", x).then(res => {
					this.tableData = res.data.data.map(item => {
						item.majorVariety = this.switchMajorVariety(item.majorVariety);
						return item
					})
		...
		mounted() {
			...
			axios('/api/' + this.studentId + '/getSchoolSubmitById').then(res => {
				this.tableData = res.data.data.map(item => {
					item.majorVariety = this.$options.methods.switchMajorVariety(item.majorVariety);
					return item
				})
				this.loading = false
			})
