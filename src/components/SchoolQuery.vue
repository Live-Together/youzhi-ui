	...
	<div style="padding-top:20px;">
			...
			<el-row :gutter="20">
				<el-col :span="8">
					<div class="grid-content bg-purple">
						<el-input v-model="target.schoolName" placeholder="输入学校名称" @keyup.enter.native="handleSearch">
							<template #suffix>
								<el-button type="text" icon="el-icon-search" id="search"
				...
				<el-col :span="4">
					<div class="grid-content bg-purple">
						<el-select v-model="target.majorName" filterable placeholder="专业名称" clearable>
							<el-option v-for="item in filter.majorName" :key="item" :value="item">
							</el-option>
						</el-select>
					</div>
				</el-col>
				<el-col :span="4">
					<div class="grid-content bg-purple">
						<el-select v-model="target.year" filterable placeholder="年份" clearable>
				...
				<el-col :span="4">
					<div class="grid-content bg-purple">
						<el-select v-model="target.majorVariety" placeholder="招收对象" clearable>
							<el-option v-for="item in filter.majorVariety" :key="item" :value="item"
				...
				<el-col :span="4">
					<div class="grid-content bg-purple">
						<el-select v-model="target.schoolVariety" placeholder="学校类别" clearable>
							<el-option v-for="item in filter.schoolVariety" :key="item" :value="item"
		...
		<div class="table">
			<el-table v-loading="isLoading" :data="tableData" stripe style="width: 100%">
				...
				<el-table-column v-for="item in menu" :key="item.prop" :label="item.label" :prop="item.prop">
				</el-table-column>
			</el-table>
		</div>
	...
	export default {
		...
		methods: {
			handleSearch() {
				...
				this.getSchoolInfo();
				axios.post('/api/getSchoolInfoCount', this.target).then((res) => {
					this.total = res.data.data;
				});

				// 获取过滤条件
				if (this.target.schoolName !== null && this.target.schoolName !== '') {
					axios.get('/api/getSchoolInfoBySchoolName', {
						params: {
							"schoolName": this.target.schoolName
						}
					}).then(res => {
					...
					[this.filter.majorName, this.filter.year, this.filter.majorVariety, this.filter.schoolVariety] = [
						[],
						['2022', '2021', '2020', '2019'],
						[1, 2, 3],
						[1, 2, 3, 4, 5]
			...
			getSchoolInfo() {
				...
				axios.get("/api/getSchoolInfo", {
						params: {
							...
							schoolVariety: this.target.schoolVariety,
							majorVariety: this.target.majorVariety,
						},
					})
					.then((res) => {
