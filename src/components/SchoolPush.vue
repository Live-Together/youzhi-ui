<template>
	<div id="app">
				...
				<el-col :span="12">
					<div class="item">
						...
						<el-select v-model="schoolVariety" placeholder="可选" clearable>
							<el-option v-for="item in schoolVarietyFilter" :key="item" :value="item"
								:label="switchSchoolVariety(item)">
							</el-option>
						</el-select>
					</div>
					<div class="item">
						<el-button type="primary" style="margin-left: 30px; width: 200px"
		...
		<div id="list">
			<el-table :data="tableList" style="width: 90%; margin: 0 auto" v-loading="loading">
					...
					<template slot-scope="scope">
						<el-row :gutter="20">
							...
							<el-col :span="16">
								...
								<div style="margin-top: 15px">{{ switchMajorVariety(scope.row.majorVariety) }} |
									{{ switchSchoolVariety(scope.row.schoolVariety) }} |
									{{ scope.row.majorName }}&nbsp;&nbsp;&nbsp;&nbsp;最低分：{{ scope.row.scoreLine }}</div>
							</el-col>
						</el-row>
					</template>
				</el-table-column>
