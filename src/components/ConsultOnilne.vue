    ...
    <el-row>
        <el-col :span="4">
            <el-card style="width: 300px; height: 300px; color: #333">
                <div style="padding-bottom: 10px; border-botton: 1px solid #ccc">
                    在线用户<span style="font-size: 12px">(点击聊天气泡开始聊天)</span>
                </div>
                <div style="padding: 10px 0" v-for="user in users" :key="user.username">
                    <span>{{ user.username }}</span>
                    <i class="el-icon-chat-round" style="margin-left: 10px; font-size: 16px; cursor: pointer" @click="chatUser = user.username"></i>
    ...
    methods: {
        ...
        createContent(remoteUser,nowUser,text) {
            ...
            if (nowUser){
                ...
                "   <span class=\"el-avatar el-avatar--circle\" style=\"height: 40px; width: 40px; line-height: 40px;\">\n"+
                "       <img src=\"https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726flepng.png\"style=\"object-fit:cover;\">\n"+
                "   </span>\n" +
                "   </div>\n" +
        ...
        init() {
            ...
            let username = this.$store.state.id;
            let _this = this
            if (typeof (WebSocket)=="undefined"){
                console.log("您的浏览器不支持WebSocket");
            } else {
                console.log("您的浏览器支持WebSocket");
                let socketUrl = "ws://localhost:8000/chat/" + username;
                if (socket){
                    socket.close();
                    socket = null;
                }
